					      ...
					      AgsRecyclingContext *recycling_context)
{
  ...
  void ags_channel_recursive_reset_recycling_context_up(AgsChannel *channel){
    ...
    while(channel != NULL){
      ...
      ags_channel_reset_recycling_context(channel->recall_id);
  ...
  void ags_channel_recursive_reset_recycling_context_down_input(AgsChannel *output){
	...
	ags_channel_reset_recycling_context(recall_id);

	ags_channel_recursive_reset_recycling_context_down_output(input->link);
				    ...
				    AgsRecallID *recall_id, gint stage)
{
  ...
  void ags_channel_recursive_play_input_sync_threaded(AgsChannel *input, AgsRecallID *input_recall_id){
    ...
    if(input->link != NULL){
      ...
      child_position = ags_recycling_context_find_child(input_recall_id->recycling_context,
							input->first_recycling);
      
      if(child_position == -1){
	child_recall_id = ags_recall_id_find_recycling_context(input->link->recall_id,
							       input_recall_id->recycling_context);
      }else{
	GList *list;

	list = g_list_nth(input_recall_id->recycling_context->children,
			  child_position);

	if(list != NULL){
	  child_recall_id = ags_recall_id_find_recycling_context(AGS_AUDIO(input->link)->recall_id,
  ...
  void ags_channel_recursive_play_input_async_threaded(AgsChannel *input,
    ...
    while(input != NULL){
      ...
      child_position = ags_recycling_context_find_child(default_recall_id->recycling_context,
							input->first_recycling);
	...
	list = g_list_nth(default_recall_id->recycling_context->children,
			  child_position);
  ...
  void ags_channel_recursive_play_output_threaded(AgsChannel *output, AgsRecallID *output_recall_id){
    ...
    if((AGS_AUDIO_ASYNC & (audio->flags)) != 0){
      ...
      /* input_recall_id - check if there is a new recycling */
      child_position = ags_recycling_context_find_child(output_recall_id->recycling_context,
							input->first_recycling);
      
      if(child_position == -1){
	default_recall_id = ags_recall_id_find_recycling_context(audio->recall_id,
								 output_recall_id->recycling_context);
      }else{
	GList *list;

	list = g_list_nth(output_recall_id->recycling_context->children,
			  child_position);
	...
	if(list != NULL){
      ...
      /* call audio */
      if(default_recall_id->recycling_context != output_recall_id->recycling_context){
 ...
 *
 * Make the tree ready for a new #AgsPlayback.
 *
 * Since: 0.3
 */
AgsRecallID*
				       ...
				       AgsChannel *old_channel_link, AgsChannel *old_link_link)
{
  ...
  GList* ags_channel_tillrecycling_collect_devout_play_down_input(AgsChannel *output,
	...
	/* follow the links */
	list = ags_channel_tillrecycling_collect_playback_down(current->link,
							...
							GList *invalid_recall_id_list)
  {
	...
	ags_channel_recursive_reset_recall_id_down(current->link,
						   next_recall_id_list, playback_list,
  ...
  channel_recall_id_list = ags_channel_tillrecycling_collect_playback_down(channel,
									   channel_recall_id_list, TRUE);
  
  /* retrieve upper */
  link_recall_id_list = NULL;
  link_recall_id_list = ags_channel_recursive_collect_playback_up(link,
								  TRUE);
  ...
  channel_invalid_recall_id_list = ags_channel_tillrecycling_collect_playback_down(old_link_link,
										   channel_invalid_recall_id_list,
										   TRUE);

  /* retrieve invalid upper */
  link_invalid_recall_id_list = NULL;
  link_invalid_recall_id_list = ags_channel_recursive_collect_playback_up(old_channel_link,
									  TRUE);

  /* go down */
  ags_channel_recursive_reset_recall_id_down(channel,
					     link_recall_id_list, link_playback_list,
					     channel_invalid_recall_id_list);
  ...
  ags_channel_tillrecycling_reset_recall_id_up(link,
					       channel_recall_id_list, channel_playback_list,
  ...
  /* free the lists */
  g_list_free(channel_playback_list);
  g_list_free(link_playback_list);
  ...
  case TRUE:
    ...
    while(current != NULL){
      recall_id = ags_recall_id_find_parent_recycling_context(current->recall_id,
							      default_recall_id->recycling_context->parent);
